...
plot_results <- function(baseline, equality, reputation, confirmation) {
  ...
  data_summary$condition <- tools::toTitleCase(data_summary$condition)
  ...
  difficulty_plot <- ggplot(data_summary, aes(x = difficulty, y = mean_group_answer, color = condition)) +
    ...
    theme_minimal() +
    scale_color_manual(values=c("#eb5146", "#21b9ff", "#bc21ff", "#23de8d")) +
  ...
  knowledge_spread_plot <- ggplot(data, aes(x = knowledge_sd, y = group_performance, color = condition)) +
    geom_smooth(method = 'loess', se = TRUE, size = 1.3) + 
    scale_color_manual(values=c("#eb5146", "#21b9ff", "#bc21ff", "#23de8d")) +
    theme_minimal() +
    labs(title = 'Group Performance over Spread of Intelligence (SD)',
         x = "SD of Group Intelligence",
         y = "Mean Group Performance")
