...
# simulation
n_trials              <- 75
group_strategy        <- 'highest_weight'  # c('highest_weight', 'highest_weight')
difficulty_mu         <- 1.0
difficulty_sd         <- 0.25
prob_error            <- 0.05
...
for (i in c('baseline', 'equality', 'reputation', 'confirmation')) {
  message(paste(i, ":"))
  message(paste("Mean: ", mean(data$group_performance[data$trial_n==max(data$trial_n) & data$condition == i])))
  message(paste("SD:   ", sd(data$group_performance[data$trial_n==max(data$trial_n) & data$condition == i])))
  message(paste("\n"))
...
####### Could be cool to do ridgeline with these, sorted by trial difficulty
ggplot(data[data$difficulty==1,], aes(x = prob_correct)) +
  geom_density()

ggplot(data[data$difficulty==1,], aes(x = confidence)) +
  geom_density()


ggplot(data[data$difficulty==1.2,], aes(x = confidence, y = knowledge_scaled, color = agent_id)) +
  geom_point()
