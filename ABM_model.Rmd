...
# simulation
n_trials              <- 75
group_strategy        <- 'sample_weighted'  # c('sample_weighted', 'highest_weight')
difficulty_mu         <- 1.0
difficulty_sd         <- 0.3
prob_error            <- 0.01
...
sim_baseline <- sim_baseline %>%
  group_by(group_id) %>% 
  mutate(knowledge_group_sd = sd(knowledge), group_performance = mean(sum(group_answer) / 3))


ggplot(sim_baseline, aes(x = knowledge_group_sd, y = group_performance)) +
  geom_smooth()

lm(as.data.frame(sim_baseline[sim_baseline$trial_n == 1,]), group_performance ~ knowledge_group_sd)
...
sim_baseline <- sim_baseline %>% 
  group_by(group_id) %>%
  mutate(group_performance = sum(group_answer / 3))

sim_reputation <- sim_reputation %>% 
  group_by(group_id) %>%
  mutate(group_performance = sum(group_answer / 3))

sim_confirmation <- sim_confirmation %>% 
  group_by(group_id) %>%
  mutate(group_performance = sum(group_answer / 3))


sim_baseline$Condition <- "Baseline"
sim_reputation$Condition <- "Reputation"
sim_confirmation$Condition <- "Confirmation"




summary <- rbind(sim_baseline[sim_baseline$trial_n == max(sim_baseline$trial_n), ],
                 sim_reputation[sim_reputation$trial_n == max(sim_reputation$trial_n), ],
                 sim_confirmation[sim_confirmation$trial_n == max(sim_confirmation$trial_n), ])




ggplot(summary, aes(x = group_performance, color = Condition, fill=Condition)) + 
  geom_density(size=0.9, alpha=.3) +
  theme_minimal() + 
  labs(title = 'Density of Group Performance over Condition',
       x     = 'Group Performance',
       y     = 'Density')
